...
class ACseRemind:
    ...
    def layarlogin(self,user_or_owner):
        ...
        if user_or_owner == 1:
            ...
            self.button_logout=Button(window,text="Back",bg = "#ecf0f3", font=("sans serif",10,"bold underline"),fg="red",command=self.logout2,
...
#==========ADMIN DASHBOARD=======#
    def owner_dashboard(self):
        ...
        self.button_logout=Button(window,text="Log out",bg = "#ecf0f3", font=("sans serif",10,"bold underline"),fg="red",command=self.logout,
                                     activebackground='red',highlightbackground="#75bfd5",highlightcolor="#75bfd5",highlightthickness=0,border=0,borderwidth=0).place(x=60,y=70)
    def logout2(self):
        logout = messagebox.askyesno("Back","Apakah anda ingin keluar?")
    ...
    def owner(self, owner_id):
        ...
        Servis_Cuci = Label(self.frame_owner, justify='center', text=service_names[owner_id],
                            font=("Arial Rounded MT Bold", 11, "bold"), bg='#f8f8f8', fg="#072D44")
        ...
        with open('booking_data.csv', mode='r') as file:
            csv_reader = csv.DictReader(file)
            for row in csv_reader:
                if row['Service Name'] == service_names[owner_id]:
                    list_booking.append(row)

        text_widget = scrolledtext.ScrolledText(self.frame_owner, wrap='word', width=60, height=20,
                                                font=("Arial", 10), bg='#f8f8f8', fg="#072D44")
        text_widget.place(x=35, y=110)

        if not list_booking:
            no_booking_label = Label(self.frame_owner, text="Tidak ada orang yang menggunakan jasa anda",
                                     font=("Arial", 10), bg='#f8f8f8', fg="#072D44")
            no_booking_label.place(x=80, y=170)
        else:
            for booking in list_booking:
                status_var = IntVar()
                status_checkbox = Checkbutton(self.frame_owner, text="Serviced", variable=status_var)
                status_checkbox.grid(row=list_booking.index(booking), column=0)
        ...
        ctk.CTkButton(self.frame_owner, text="Back", corner_radius=20, bg_color='#f8f8f8', hover_color='red',
                      fg_color='#1679ad', width=100, height=30, command=self.frame_owner.destroy).place(x=50, y=460)
...
#====================REMINDER=========================#
    ...
    def atur_tanggal_kirim_email(self):
        ...
        with open('dataremind.csv', 'a', newline='') as file:
            ...
            def add_three_months(input_date):
                    ...
                    new_date = date_object + timedelta(days=0)
        ...
        self.sender_email = "alishadeana.23244@mhs.unesa.ac.id"
        self.sender_password = "pluwvhhi"
        

        sent_from = self.sender_email
        sent_to = self.emailRemind.get()
        subject = self.emailSubject
        body = f" Dear customer, thank you for using our apps, your last service is {self.mydate}, reminder has sucessfully set, we'll remind you to service your AC three months after your last service \n\nwith note: {self.noteRemind.get()}"
        ...
        try:
            ...
            print("email set dikirim\nemail reminder segara terkirim")
            # self.send_notification()
            # self.set_reminder()
            self.send_email_after()
    ...
    def send_notification(self):
        ...
        self.sender_email = "alishadeana.23244@mhs.unesa.ac.id"
        self.sender_password = "pluwvhhi"
        self.recipient_email = self.emailRemind.get()

        try:

            subject = "Reminder"
            body = f"Waktunya Service AC Anda. Reminder diatur pada {self.mydate}."
            ...
            # Konfigurasi server SMTP (gunakan server dan port email provider Anda)
            ...
            # print(f"Notifikasi email terkirim pada {datetime.now()+ timedelta(minutes=5)} ke {self.recipient_email}.")
            
        except Exception as e:
            print(f"Error sending email: {e}")

    def send_notification2(self):
        ...
        try:
            ...
            subject = "Reminder Confirmation: Pekerjaan yang perlu dilakukan"
            body = f"Konfirmasi bahwa reminder untuk pekerjaan Anda sudah diatur pada {self.mydate}. Anda akan menerima notifikasi lagi setelah 90 hari."
    ...
    def send_email_after(self):
        # Jalankan fungsi pengiriman email dalam thread terpisah
        threading.Timer(300, self.send_notification).start()

        # ninety_days_later = datetime.now() + timedelta(days=90)
        # threading.Timer((ninety_days_later - datetime.now()).total_seconds(), self.send_notification).start()
        
    # def set_reminder(self):
            # try:
            # reminder_time = datetime.now()+ timedelta(minutes=5)
            #     # Jadwalkan pengiriman notifikasi 10 menit setelah waktu pengingat
            # schedule.every().day.at((reminder_time).strftime("%H:%M")).do(self.send_notification)
    ...
    # def set_reminder(self):
        # # Waktu pengingat pada hari ini pukul 13:00
        # reminder_time = datetime.now()+ timedelta(minutes=5)
